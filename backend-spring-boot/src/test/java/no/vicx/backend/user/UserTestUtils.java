...
public final class UserTestUtils {
    ...
    static UserVm createUserVm(
            String username, String password, String email, String name, String recaptchaToken) {
        return new UserVm(username, password, email, name, recaptchaToken);
    }
    ...
    static Stream<Arguments> invalidUserVmProvider() {
        return Stream.of(
                ...
                Arguments.of(
                        createUserVm("user1", null, "user@example.com", "The User", "mock-token"),
                        "password", "Cannot be null"
                ),
                Arguments.of(
                        createUserVm("user1", "Aa1Aa1", "user@example.com", "The User", "mock-token"),
                        "password", "It must have minimum 8 and maximum 255 characters"
                ),
                Arguments.of(
                        createUserVm("user1", "Aa1".repeat(90), "user@example.com", "The User", "mock-token"),
                        "password", "It must have minimum 8 and maximum 255 characters"
                ),
                Arguments.of(
                        createUserVm("user1", "a".repeat(8), "user@example.com", "The User", "mock-token"),
                        "password", "Password must have at least one uppercase, one lowercase letter and one number"
                ),

                Arguments.of(
                        createUserVm("user1", "P4ssword", null, "The User", "mock-token"),
                        "email", "Cannot be null"
                ),
                Arguments.of(
                        createUserVm("user1", "P4ssword", "a", "The User", "mock-token"),
                        "email", "It must be a well-formed email address"
                ),

                Arguments.of(
                        createUserVm("user1", "P4ssword", "user@example.com", null, "mock-token"),
                        "name", "Cannot be null"
                ),
                Arguments.of(
                        createUserVm("user1", "P4ssword", "user@example.com", "a".repeat(3), "mock-token"),
                        "name", "It must have minimum 4 and maximum 255 characters"
                ),
                Arguments.of(
                        createUserVm("user1", "P4ssword", "user@example.com", "a".repeat(256), "mock-token"),
                        "name", "It must have minimum 4 and maximum 255 characters"
                ),
