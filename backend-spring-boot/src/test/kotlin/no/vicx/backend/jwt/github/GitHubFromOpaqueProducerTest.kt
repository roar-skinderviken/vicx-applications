...
class GitHubFromOpaqueProducerTest : BehaviorSpec({
    ...
    Given("createPrincipal, valid GitHub token response") {
        every { githubUserFetcher.fetchUser(any()) } returns GitHubUserResponseVm(
    ...
    Given("createPrincipal, failure scenarios") {
        When("calling createPrincipal with invalid token") {
            every { githubUserFetcher.fetchUser(any()) } throws HttpClientErrorException.create(
            ...
            val thrown = shouldThrow<BadOpaqueTokenException> {
